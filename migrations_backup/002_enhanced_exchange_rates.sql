...
-- Use safe enum update approach
CREATE TYPE currency_type_new AS ENUM ('USD', 'THB', 'EUR', 'GBP', 'SGD', 'VND', 'MYR', 'BTC');

-- 3. Update columns to use new enum type
ALTER TABLE exchange_rates 
  ALTER COLUMN from_currency TYPE currency_type_new USING from_currency::text::currency_type_new,
  ALTER COLUMN to_currency TYPE currency_type_new USING to_currency::text::currency_type_new;

-- Also update other tables that use currency_type
ALTER TABLE transactions 
  ALTER COLUMN original_currency TYPE currency_type_new USING original_currency::text::currency_type_new;

-- For users table, need to handle the default value
ALTER TABLE users 
  ALTER COLUMN preferred_currency DROP DEFAULT,
  ALTER COLUMN preferred_currency TYPE currency_type_new USING preferred_currency::text::currency_type_new,
  ALTER COLUMN preferred_currency SET DEFAULT 'USD'::currency_type_new;
