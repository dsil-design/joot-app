...
// Helper function to parse date in format "Monday, November 1, 2024"
...
// Helper function to parse date in format "11/23/2024" or "11/23/2024 12:00 AM"
function parseShortDate(dateStr) {
  // Handle date with time (e.g., "11/23/2024 12:00 AM")
...
console.log('========================================');
console.log('NOVEMBER 2024 PARSING SCRIPT');
console.log('========================================\n');

// Section 1: Expense Tracker (lines 3403-3589)
console.log('Parsing Expense Tracker (lines 3403-3589)...');
let currentDate = null;

for (let i = 3402; i < 3590; i++) {
...
// Section 2: Gross Income Tracker (lines 3584-3590)
console.log('\nParsing Gross Income Tracker (lines 3584-3590)...');

for (let i = 3583; i < 3591; i++) {
  const row = parseCSV(lines[i]);
...
// Section 3: Personal Savings & Investments (lines 3594-3596)
console.log('\nParsing Personal Savings & Investments (lines 3594-3596)...');

for (let i = 3593; i < 3597; i++) {
  const row = parseCSV(lines[i]);
  ...
  // Parse date (format: 11/23/2024 or 11/23/2024 12:00 AM)
  ...
  if (!date) {
    date = '2024-11-30'; // Default to last day of November
  }
...
for (let i = 3607; i < 3618; i++) {
  ...
  if (currentDate === '2024-11-30' && !parseFullDate(row[0])) {
...
// Find Line 3408 (Florida House $1,000 transfer) for verification
...
const refunds = transactions.filter(t =>
  (t.description && (t.description.toLowerCase().includes('refund') || t.merchant.toLowerCase() === 'apple' || t.merchant.toLowerCase() === 'amazon')) &&
...
if (rentTransaction) {
  ...
  if (rentTransaction.currency === 'THB' && Math.abs(rentTransaction.amount - 25000) < 1) {
    console.log('  ✅ CORRECT - Rent is 25000 THB (NOT 740 USD)');
  } else {
...
// Verify Line 3408 (Florida House $1,000 transfer with comma formatting)
if (floridaHouseTransfer) {
  console.log('✓ LINE 3408 VERIFICATION (Comma-Formatted Amount):');
  ...
  console.log('⚠️  WARNING: Line 3408 (Florida House transfer) transaction not found!');
}
...
if (refunds.length > 0) {
  ...
  console.log('⚠️  INFO: No refund transactions found (expected 3)');
}
...
const outputPath = '/Users/dennis/Code Projects/joot-app/scripts/november-2024-CORRECTED.json';
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));
console.log(`✅ Written: ${outputPath}`);

// 2. Generate parsing report
const reportPath = '/Users/dennis/Code Projects/joot-app/scripts/NOVEMBER-2024-PARSE-REPORT.md';
let report = `# NOVEMBER 2024 PARSING REPORT
...
- [${transactions.length >= 115 && transactions.length <= 125 ? 'x' : ' '}] Transaction count in expected range (115-125)
...
- [${floridaHouseTransfer && Math.abs(floridaHouseTransfer.amount - 1000) < 0.01 ? 'x' : ' '}] Line 3408 verification passed ($1,000.00)
- [${currencyDistribution.USD > 0 && currencyDistribution.THB > 0 ? 'x' : ' '}] Both USD and THB transactions present
- [${negativeConversions.length >= 3 ? 'x' : ' '}] Negative amounts converted (${negativeConversions.length})
...
- [${tagDistribution['Business Expense'] || 0 >= 10 ? 'x' : ' '}] Business Expense tag count (${tagDistribution['Business Expense'] || 0})
...
${verificationPassed && transactions.length >= 115 && transactions.length <= 125 &&
...
const redFlagsPath = '/Users/dennis/Code Projects/joot-app/scripts/NOVEMBER-2024-RED-FLAGS.md';

// Create new red flags file
let redFlagsContent = `# NOVEMBER 2024 RED FLAGS AND DISCREPANCIES
...
${corrections.length > 0 ? corrections.map((c, idx) => `
...
- Line 3408: ${floridaHouseTransfer ? `$${floridaHouseTransfer.amount} USD` : 'NOT FOUND'} ${floridaHouseTransfer && Math.abs(floridaHouseTransfer.amount - 1000) < 0.01 ? '✓ (comma-formatted)' : '✗'}
...
${verificationPassed && transactions.length >= 115 && transactions.length <= 125 && negativeTransactions.length === 0 ? '✅ **YES** - Ready to import to database' : '⚠️ **REVIEW REQUIRED** - Check verification failures'}
...
console.log('\nFINAL STATUS:');
console.log(`  Total Transactions: ${transactions.length}`);
console.log(`  Expected Range: 115-125`);
console.log(`  User-Confirmed Corrections: ${corrections.length}`);
...
console.log(`  READY FOR IMPORT: ${verificationPassed && transactions.length >= 115 && transactions.length <= 125 && negativeTransactions.length === 0 ? '✅ YES' : '⚠️ REVIEW REQUIRED'}`);
