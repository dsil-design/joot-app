...
/**
 * FEBRUARY 2025 COMPREHENSIVE VALIDATION SCRIPT
 *
...
async function validateLevel1(userId) {
  ...
  const { data: expenseTrackerTxns } = await supabase
    .from('transactions')
    .select('*')
    .eq('user_id', userId)
    .gte('date', '2025-02-01')
    .lte('date', '2025-02-28')
    .not('tags', 'cs', '["Florida House"]')
    .not('tags', 'cs', '["Savings/Investment"]');
  ...
  for (const txn of expenseTrackerTxns || []) {
    ...
    } else if (txn.transaction_type === 'income' && txn.tags?.includes('Reimbursement')) {
  ...
  level1Results.expenseTracker = {
    ...
    transactionCount: expenseTrackerTxns?.length || 0
  };
  ...
  const { data: floridaHouseTxns } = await supabase
    .from('transactions')
    .select('*')
    .eq('user_id', userId)
    .gte('date', '2025-02-01')
    .lte('date', '2025-02-28')
    .contains('tags', ['Florida House']);
  ...
  level1Results.floridaHouse = {
    ...
    transactionCount: floridaHouseTxns?.length || 0
  };
  ...
  const { data: savingsTxns } = await supabase
    ...
    .gte('date', '2025-02-01')
    .lte('date', '2025-02-28')
    .contains('tags', ['Savings/Investment']);

  const savingsTotal = savingsTxns?.length || 0;
  ...
  level1Results.grossIncome = {
    ...
    pass: incomePass,
    transactionCount: incomeTxns?.length || 0
  };
...
async function validateLevel2(userId) {
  ...
  for (const [date, pdfTotal] of Object.entries(PDF_DAILY_TOTALS)) {
    ...
    dailyResults.push({
      ...
      status,
      transactionCount: dayTxns?.length || 0
    });
  }
...
async function validateLevel3(userId) {
  ...
  const byType = {
    expense: allTxns?.filter(t => t.transaction_type === 'expense').length || 0,
    income: allTxns?.filter(t => t.transaction_type === 'income').length || 0
  };
  const byCurrency = {
    USD: allTxns?.filter(t => t.currency === 'USD').length || 0,
    THB: allTxns?.filter(t => t.currency === 'THB').length || 0
  };
  const bySection = {
    ...
    floridaHouse: allTxns?.filter(t => t.tags?.includes('Florida House')).length || 0,
    savings: allTxns?.filter(t => t.tags?.includes('Savings/Investment')).length || 0,
...
async function validateLevel4(userId) {
  ...
  const tagCounts = {
    reimbursement: allTxns?.filter(t => t.tags?.includes('Reimbursement')).length || 0,
    floridaHouse: allTxns?.filter(t => t.tags?.includes('Florida House')).length || 0,
    businessExpense: allTxns?.filter(t => t.tags?.includes('Business Expense')).length || 0,
    savings: allTxns?.filter(t => t.tags?.includes('Savings/Investment')).length || 0
  };
...
async function validateLevel5(userId) {
  ...
  const { data: rentTxn } = await supabase
    .from('transactions')
    .select('*')
    .eq('user_id', userId)
    .eq('date', '2025-02-05')
  ...
  const rentPass = rentTxn &&
                   ...
                   rentTxn.currency === 'THB' &&
                   rentTxn.merchant === 'Pol';

  checks.push({
    name: 'Rent Transaction',
    expected: { description: "This Month's Rent", amount: 25000, currency: 'THB', date: '2025-02-05', merchant: 'Pol' },
    found: rentTxn ? { description: rentTxn.description, amount: rentTxn.amount, currency: rentTxn.currency, date: rentTxn.date, merchant: rentTxn.merchant } : null,
    pass: rentPass
  });
  ...
  const { data: floridaTxns } = await supabase
    .from('transactions')
    .select('*')
    .eq('user_id', userId)
    .eq('date', '2025-02-28')
  ...
  const waterPass = waterBill && waterBill.amount === 54.8 && waterBill.currency === 'USD';
  const gasPass = gasBill && gasBill.amount === 36.49 && gasBill.currency === 'USD';
  ...
  const { data: typoReimb } = await supabase
    .from('transactions')
    .select('*')
    .eq('user_id', userId)
    .gte('date', '2025-02-01')
    .lte('date', '2025-02-28')
    .contains('tags', ['Reimbursement'])
    .or('description.ilike.%Remibursement%,description.ilike.%Rembursement%,description.ilike.%Reimbursment%');
  ...
  checks.push({
    ...
    found: { count: typoReimb?.length || 0, found: typoReimb?.map(t => t.description) },
  ...
  const { data: golfWinnings } = await supabase
    .from('transactions')
    .select('*')
    .eq('user_id', userId)
    .eq('date', '2025-02-09')
    .eq('description', 'Golf Winnings')
    .single();

  const golfPass = golfWinnings &&
                   golfWinnings.amount === 500 &&
                   golfWinnings.currency === 'THB' &&
  ...
  const { data: floridaHouseTxn } = await supabase
    .from('transactions')
    .select('*')
    .eq('user_id', userId)
    .eq('date', '2025-02-01')
    .eq('description', 'Florida House')
    .single();

  const commaPass = floridaHouseTxn &&
                    floridaHouseTxn.amount === 1000 &&
                    floridaHouseTxn.currency === 'USD';
  ...
  const { data: allTHB } = await supabase
    .from('transactions')
    .select('*')
    .eq('user_id', userId)
    .gte('date', '2025-02-01')
  ...
  const largestTHB = allTHB?.[0];
  console.log(`   Largest: ${largestTHB?.description} - ${largestTHB?.amount} THB on ${largestTHB?.date}`);
  ...
  const largestUSD = allUSD?.[0];
  console.log(`   Largest: ${largestUSD?.description} - $${largestUSD?.amount} on ${largestUSD?.date}`);
...
async function validateLevel6(userId) {
  ...
  // This level requires manual PDF extraction
  // For now, we'll provide a framework and sample
  ...
  // Get all February transactions ordered by date and amount
  const { data: allTxns } = await supabase
    .from('transactions')
    .select('*')
    .eq('user_id', userId)
  ...
  for (const txn of allTxns || []) {
    const entry = {
      ...
      usdAmount: convertToUSD(txn.amount, txn.currency).toFixed(2),
      type: txn.transaction_type,
      tags: txn.tags || []
    };

    if (txn.tags?.includes('Florida House')) {
      level6Data.floridaHouse.push(entry);
    } else if (txn.tags?.includes('Savings/Investment')) {
  ...
  console.log(`   Total: ${allTxns?.length || 0}\n`);
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════════╗');
  console.log('║     FEBRUARY 2025 COMPREHENSIVE VALIDATION                     ║');
  ...
  try {
    ...
    fs.writeFileSync(
      '/Users/dennis/Code Projects/joot-app/scripts/february-2025-validation-results.json',
      JSON.stringify({ summary, ...results }, null, 2)
    );

    console.log('Full results saved to: scripts/february-2025-validation-results.json\n');
