...
/**
 * JUNE 2022 PARSING SCRIPT
 * Enhanced with 25+ months of historical learnings
 *
 * Protocol: MASTER-IMPORT-PROTOCOL v3.0 + Protocol v2.0
 * Batch: Aug-Jul-Jun-May 2022 (Month 3 of 4)
 *
 * Line Ranges from CSV:
 * - Expense Tracker: 11039-11191 (~153 lines)
 * - Gross Income Tracker: 11192-11199 (~8 lines)
 * - Personal Savings & Investments: 11201-11205 (~5 lines)
 */

console.log('========================================');
console.log('JUNE 2022 PARSING SCRIPT');
console.log('========================================\n');
console.log('Protocol: MASTER-IMPORT-PROTOCOL v3.0');
console.log('Enhanced with 25+ months of historical learnings');
console.log('Batch: Aug-Jul-Jun-May 2022 (Month 3 of 4)\n');
...
function parseFullDate(dateStr) {
  ...
  if (match) {
    ...
    if (monthName === 'June' && year !== '2022') {
      console.log(`  ⚠️  DATE TYPO DETECTED: ${dateStr} - Expected June 2022`);
      parsedYear = '2022';
      console.log(`  ✓ DATE TYPO CORRECTED: ${dateStr} → June ${day}, 2022`);
    }
...
// Section 1: Expense Tracker (lines 11039-11191)
console.log('Parsing Expense Tracker (lines 11039-11191)...');
let currentDate = null;

for (let i = 11038; i < 11192; i++) {
  ...
  const transaction = {
    transaction_date: currentDate || '2022-06-30',
    description,
    merchant,
    amount,
    currency,
...
// Section 2: Gross Income Tracker (lines 11192-11199)
console.log('Parsing Gross Income Tracker (lines 11192-11199)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2022-06-30';

for (let i = 11192; i < 11200; i++) {
  const row = parseCSV(lines[i]);
...
// Section 3: Personal Savings & Investments (lines 11201-11205)
console.log('Parsing Personal Savings & Investments (lines 11201-11205)...');

// Set default date to last day of month for savings
let savingsDate = '2022-06-30';

for (let i = 11201; i < 11206; i++) {
  const row = parseCSV(lines[i]);
...
// Write output file
const outputPath = path.join(__dirname, 'june-2022-CORRECTED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file
const metadataPath = path.join(__dirname, 'june-2022-METADATA.json');
const metadata = {
  month: 'June 2022',
...
console.log('1. Review june-2022-CORRECTED.json');
