...
/**
 * JULY 2022 PARSING SCRIPT
 * Enhanced with 25+ months of historical learnings
 *
 * Protocol: MASTER-IMPORT-PROTOCOL v3.0 + Protocol v2.0
 * Batch: Aug-Jul-Jun-May 2022 (Month 2 of 4)
 *
 * Line Ranges from CSV:
 * - Expense Tracker: 10813-11006 (~194 lines)
 * - Gross Income Tracker: 11007-11021 (~15 lines)
 * - Personal Savings & Investments: 11023-11027 (~5 lines)
 */

console.log('========================================');
console.log('JULY 2022 PARSING SCRIPT');
console.log('========================================\n');
console.log('Protocol: MASTER-IMPORT-PROTOCOL v3.0');
console.log('Enhanced with 25+ months of historical learnings');
console.log('Batch: Aug-Jul-Jun-May 2022 (Month 2 of 4)\n');
...
function parseFullDate(dateStr) {
  ...
  if (match) {
    ...
    if (monthName === 'July' && year !== '2022') {
      console.log(`  ⚠️  DATE TYPO DETECTED: ${dateStr} - Expected July 2022`);
      parsedYear = '2022';
      console.log(`  ✓ DATE TYPO CORRECTED: ${dateStr} → July ${day}, 2022`);
    }
...
// Section 1: Expense Tracker (lines 10813-11006)
console.log('Parsing Expense Tracker (lines 10813-11006)...');
let currentDate = null;

for (let i = 10812; i < 11007; i++) {
  ...
  const transaction = {
    transaction_date: currentDate || '2022-07-31',
    description,
    merchant,
    amount,
    currency,
...
// Section 2: Gross Income Tracker (lines 11007-11021)
console.log('Parsing Gross Income Tracker (lines 11007-11021)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2022-07-31';

for (let i = 11007; i < 11022; i++) {
  const row = parseCSV(lines[i]);
...
// Section 3: Personal Savings & Investments (lines 11023-11027)
console.log('Parsing Personal Savings & Investments (lines 11023-11027)...');

// Set default date to last day of month for savings
let savingsDate = '2022-07-31';

for (let i = 11023; i < 11028; i++) {
  const row = parseCSV(lines[i]);
...
// Write output file
const outputPath = path.join(__dirname, 'july-2022-CORRECTED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file
const metadataPath = path.join(__dirname, 'july-2022-METADATA.json');
const metadata = {
  month: 'July 2022',
...
console.log('1. Review july-2022-CORRECTED.json');
