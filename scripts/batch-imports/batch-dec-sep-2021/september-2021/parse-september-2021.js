...
/**
 * SEPTEMBER 2021 PARSING SCRIPT
 * Enhanced with 24+ months of historical learnings
 *
 * Protocol: MASTER-IMPORT-PROTOCOL v4.0 + Protocol v2.0
 * Batch: Dec-Sep 2021 (Month 1 of 4)
 *
 * Line Ranges from CSV:
 * - Expense Tracker: 13147-13365 (~209 lines)
 * - Gross Income Tracker: 13366-13374 (~10 lines)
 * - Personal Savings & Investments: 13376-13378 (~3 lines)
 */

console.log('========================================');
console.log('SEPTEMBER 2021 PARSING SCRIPT');
console.log('========================================\n');
console.log('Protocol: MASTER-IMPORT-PROTOCOL v4.0');
console.log('Enhanced with 24+ months of historical learnings');
console.log('Batch: Dec-Sep 2021 (Month 1 of 4)\n');
...
function parseFullDate(dateStr) {
  ...
  if (match) {
    ...
    if (monthName === 'September' && year !== '2021') {
      console.log(`  ⚠️  DATE TYPO DETECTED: ${dateStr} - Expected September 2021`);
      parsedYear = '2021';
      console.log(`  ✓ DATE TYPO CORRECTED: ${dateStr} → December ${day}, 2021`);
    }
...
// Section 1: Expense Tracker (lines 13147-13365)
console.log('Parsing Expense Tracker (lines 13147-13365)...');
let currentDate = null;

for (let i = 13146; i < 13366; i++) {
  ...
  const transaction = {
    transaction_date: currentDate || '2021-09-30',
    description,
    merchant,
    amount,
    currency,
...
// Section 2: Gross Income Tracker (lines 13366-13374)
console.log('Parsing Gross Income Tracker (lines 13366-13374)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2021-09-30';

for (let i = 13365; i < 13375; i++) {
  const row = parseCSV(lines[i]);
...
// Section 3: Personal Savings & Investments (lines 13376-13378)
console.log('Parsing Personal Savings & Investments (lines 13376-13378)...');

// Set default date to last day of month for savings
let savingsDate = '2021-09-30';

for (let i = 13375; i < 13379; i++) {
  const row = parseCSV(lines[i]);
...
const thaiRent = dualResidenceRents.find(r =>
  ...
  r.merchant.toLowerCase().includes('jatu') ||
...
const expectedCount = 190; // Estimated from line ranges (~180-200 target)
...
console.log(`  Expected: Dual residence (USA + Thailand)`);

console.log();

// Write output file
const outputPath = path.join(__dirname, 'september-2021-CORRECTED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file
const metadataPath = path.join(__dirname, 'september-2021-METADATA.json');
const metadata = {
...
console.log('1. Review september-2021-CORRECTED.json');
console.log('2. Verify dual residence rent transactions');
