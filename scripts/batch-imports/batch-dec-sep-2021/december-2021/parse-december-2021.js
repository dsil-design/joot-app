 ...
 * - Expense Tracker: 12467-12676 (~209 lines)
 * - Gross Income Tracker: 12678-12687 (~10 lines)
 * - Personal Savings & Investments: 12689-12691 (~3 lines)
 */

console.log('========================================');
console.log('DECEMBER 2021 PARSING SCRIPT');
console.log('========================================\n');
...
function parseFullDate(dateStr) {
  ...
  if (match) {
    ...
    if (monthName === 'December' && year !== '2021') {
      console.log(`  ⚠️  DATE TYPO DETECTED: ${dateStr} - Expected December 2021`);
      parsedYear = '2021';
...
// Section 1: Expense Tracker (lines 12467-12676)
console.log('Parsing Expense Tracker (lines 12467-12676)...');
let currentDate = null;

for (let i = 12466; i < 12677; i++) {
  ...
  const transaction = {
    transaction_date: currentDate || '2021-12-31',
    description,
    merchant,
    amount,
    currency,
...
// Section 2: Gross Income Tracker (lines 12678-12687)
console.log('Parsing Gross Income Tracker (lines 12678-12687)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2021-12-31';

for (let i = 12677; i < 12688; i++) {
  const row = parseCSV(lines[i]);
...
// Section 3: Personal Savings & Investments (lines 12689-12691)
console.log('Parsing Personal Savings & Investments (lines 12689-12691)...');

// Set default date to last day of month for savings
let savingsDate = '2021-12-31';

for (let i = 12688; i < 12692; i++) {
  const row = parseCSV(lines[i]);
...
// Write output file
const outputPath = path.join(__dirname, 'december-2021-CORRECTED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file
const metadataPath = path.join(__dirname, 'december-2021-METADATA.json');
const metadata = {
  month: 'December 2021',
...
console.log('1. Review december-2021-CORRECTED.json');
