...
// Section 1: Expense Tracker (lines 7450-7706)
console.log('Parsing Expense Tracker (lines 7450-7706)...');
let currentDate = null;

for (let i = 7449; i < 7707; i++) {
...
// Section 2: Gross Income Tracker (lines 7707-7715)
console.log('Parsing Gross Income Tracker (lines 7707-7715)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2023-07-31';

for (let i = 7706; i < 7716; i++) {
  const row = parseCSV(lines[i]);
...
// Section 3: Personal Savings & Investments (lines 7716-7720)
console.log('Parsing Personal Savings & Investments (lines 7716-7720)...');

// Set default date to last day of month for savings
let savingsDate = '2023-07-31';

for (let i = 7715; i < 7721; i++) {
  const row = parseCSV(lines[i]);
...
// Write output file
const outputPath = path.join(__dirname, 'july-2023-PARSED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file with exchange rate
const metadataPath = path.join(__dirname, 'july-2023-METADATA.json');
const metadata = {
  month: 'July 2023',
