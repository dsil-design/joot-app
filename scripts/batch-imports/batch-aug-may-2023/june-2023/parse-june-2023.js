...
console.log('Parsing Expense Tracker (lines 7731-7985)...');
let currentDate = null;

for (let i = 7730; i < 7986; i++) {
...
console.log('Parsing Gross Income Tracker (lines 7986-7995)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2023-06-30';

for (let i = 7985; i < 7996; i++) {
  const row = parseCSV(lines[i]);
...
console.log('Parsing Personal Savings & Investments (lines 7996-8000)...');

// Set default date to last day of month for savings
let savingsDate = '2023-06-30';

for (let i = 7995; i < 8001; i++) {
  const row = parseCSV(lines[i]);
...
// Write output file
const outputPath = path.join(__dirname, 'june-2023-PARSED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file with exchange rate
const metadataPath = path.join(__dirname, 'june-2023-METADATA.json');
const metadata = {
  month: 'June 2023',
