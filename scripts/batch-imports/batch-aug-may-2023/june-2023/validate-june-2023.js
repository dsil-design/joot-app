...
console.log('========================================');
console.log('JUNE 2023 VALIDATION QUERIES');
...
async function runValidation() {
  ...
  const { data: expenseData } = await supabase
    ...
    .eq('user_id', userId)
    .gte('transaction_date', '2023-06-01')
    .lte('transaction_date', '2023-06-30')
  ...
  const expectedCount = 192; // From parsing
  ...
  if (usaRent) {
    ...
    console.log(`  Expected: $957`);
    if (usaRent.amount === 957) {
      console.log('  ✅ Amount correct\n');
    } else {
      console.log(`  ⚠️  Amount variance: $${(usaRent.amount - 957).toFixed(2)}\n`);
    }
  } else {
  ...
  const { count: totalTagCount } = await supabase
    ...
    .gte('transactions.transaction_date', '2023-06-01')
    .lte('transactions.transaction_date', '2023-06-30');
  ...
  console.log(`Expected: 4 (from parsing)\n`);

  const expectedTagCount = 4; // From parsing output
  ...
  console.log(`Expected THB: ~58.9% (from parsing)\n`);

  const expectedThbPercent = 58.9;
