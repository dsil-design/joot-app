...
/**
 * MAY 2023 PARSING SCRIPT
 * Enhanced with 21+ months of historical learnings
 *
 * Protocol: BATCH-IMPORT-PROTOCOL-v1.2 + Monthly v3.6
 * Batch: Aug-Jul-Jun-May 2023 (Month 4 of 4)
 *
 * Line Ranges from CSV:
 * - Expense Tracker: 7450-7706 (~257 lines)
 * - Gross Income Tracker: 7707-7715 (~9 lines)
 * - Personal Savings & Investments: 7716-7720 (~5 lines)
 */

console.log('========================================');
console.log('MAY 2023 PARSING SCRIPT');
console.log('========================================\n');
console.log('Protocol: BATCH-IMPORT-PROTOCOL-v1.2 + Monthly v3.6');
console.log('Enhanced with 21+ months of historical learnings');
console.log('Batch: Aug-Jul-Jun-May 2023 (Month 4 of 4)\n');
...
// Section 1: Expense Tracker (lines 8011-8170)
console.log('Parsing Expense Tracker (lines 8011-8170)...');
let currentDate = null;

// Note: lines array is 0-indexed, so line 8014 in file = lines[8013]
for (let i = 8013; i < 8171; i++) {
  ...
  const transaction = {
    transaction_date: currentDate || '2023-05-31',
    description,
    merchant,
    amount,
    currency,
...
// Section 2: Gross Income Tracker (lines 8171-8179)
console.log('Parsing Gross Income Tracker (lines 8171-8179)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2023-05-31';

for (let i = 8170; i < 8180; i++) {
  const row = parseCSV(lines[i]);
...
// Section 3: Personal Savings & Investments (lines 8180-8184)
console.log('Parsing Personal Savings & Investments (lines 8180-8184)...');

// Set default date to last day of month for savings
let savingsDate = '2023-05-31';

for (let i = 8179; i < 8185; i++) {
  const row = parseCSV(lines[i]);
...
// Write output file
const outputPath = path.join(__dirname, 'may-2023-PARSED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file with exchange rate
const metadataPath = path.join(__dirname, 'may-2023-METADATA.json');
const metadata = {
  month: 'May 2023',
