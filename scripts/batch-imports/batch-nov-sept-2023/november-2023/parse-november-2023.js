...
console.log('========================================');
console.log('NOVEMBER 2023 PARSING SCRIPT');
console.log('========================================\n');
console.log('Protocol: BATCH-IMPORT-PROTOCOL-v1.2 + Monthly v3.6');
console.log('Batch: Nov-Oct-Sept 2023 (Month 1 of 3)\n');
...
console.log('- 5 negative refunds → income');
console.log('- 1 comma-formatted amount ($1,200 Casino)');
console.log('- 1 reimbursement expected ($99 Dinner)');
console.log('- THB%: ~2.9% (low - USA-based pattern)');
console.log('- Expected transaction count: ~105\n');

// Section 1: Expense Tracker (lines 6536-6675)
console.log('Parsing Expense Tracker (lines 6536-6675)...');
let currentDate = null;

for (let i = 6535; i < 6676; i++) {
  ...
  // November 2023: Standard columns (NO VND)
  ...
  const transaction = {
    transaction_date: currentDate || '2023-11-30',
    description,
    merchant,
    amount,
    currency,
...
// Section 2: Gross Income Tracker (lines 6676-6683)
console.log('Parsing Gross Income Tracker (lines 6676-6683)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2023-11-30';

for (let i = 6675; i < 6684; i++) {
  const row = parseCSV(lines[i]);
...
// Section 3: Personal Savings & Investments (lines 6685-6690)
console.log('Parsing Personal Savings & Investments (lines 6685-6690)...');

// Set default date to last day of month for savings
let savingsDate = '2023-11-30';

for (let i = 6684; i < 6691; i++) {
  const row = parseCSV(lines[i]);
...
// Section 4: Florida House Expenses - NOT PRESENT IN NOVEMBER 2023
console.log('Florida House Expenses: NOT PRESENT (no section in November 2023)\n');
...
// Check reimbursement count (November 2023 - Expected 1)
...
const variance = ((transactions.length - 105) / 105 * 100).toFixed(1);
...
console.log(`  Expected THB: ~2.9% (USA-based month)`);

if (parseFloat(thbPercentage) >= 1 && parseFloat(thbPercentage) <= 5) {
  console.log(`✅ THB percentage within expected range (1-5% for USA month)\n`);
} else {
...
const totalExpenses = transactions.filter(t => t.transaction_type === 'expense')
  .reduce((sum, t) => {
    if (t.currency === 'USD') {
      ...
      // For display only - use approximate rate of 0.029 for Nov 2023
...
console.log(`Expected Grand Total: ~$5,753.38`);
...
// Write output file
const outputPath = path.join(__dirname, 'november-2023-CORRECTED.json');
...
console.log('1. Review november-2023-CORRECTED.json');
console.log('2. Verify dual rent payments (Jordan $957 + Pol THB 25,000)');
console.log('3. Verify 5 negative refunds converted to income');
console.log('4. Verify 1 reimbursement tag applied');
console.log('5. Verify 1 comma-formatted amount parsed correctly ($1,200 Casino)');
console.log('6. Proceed to Phase 3: Database Import\n');
