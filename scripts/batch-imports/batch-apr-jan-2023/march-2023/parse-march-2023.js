 ...
 * Batch: Apr-Mar-Feb-Jan 2023 (Month 2 of 4)
 *
...
console.log('Batch: Apr-Mar-Feb-Jan 2023 (Month 2 of 4)\n');
...
console.log('Parsing Expense Tracker (lines 8501-8735)...');
let currentDate = null;

for (let i = 8500; i < 8736; i++) {
...
console.log('Parsing Gross Income Tracker (lines 8750-8752)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2023-03-30';

for (let i = 8749; i < 8753; i++) {
  const row = parseCSV(lines[i]);
...
console.log('Parsing Personal Savings & Investments (line 8761)...');

// Set default date to last day of month for savings
let savingsDate = '2023-03-30';

for (let i = 8760; i < 8762; i++) {
  const row = parseCSV(lines[i]);
...
// Write output file
const outputPath = path.join(__dirname, 'march-2023-PARSED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file with exchange rate
const metadataPath = path.join(__dirname, 'march-2023-METADATA.json');
const metadata = {
  month: 'March 2023',
