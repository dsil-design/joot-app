 ...
 * Batch: Apr-Mar-Feb-Jan 2023 (Month 1 of 4)
 *
 * Line Ranges from CSV:
 * - Expense Tracker: 8198-8469 (~271 lines) [CORRECTED: was 8459]
...
console.log('Batch: Apr-Mar-Feb-Jan 2023 (Month 1 of 4)\n');
...
// Section 1: Expense Tracker (lines 8198-8469)
console.log('Parsing Expense Tracker (lines 8198-8469)...');
let currentDate = null;

for (let i = 8197; i < 8470; i++) {
...
console.log('Parsing Gross Income Tracker (lines 8474-8476)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2023-04-30';

for (let i = 8473; i < 8477; i++) {
  const row = parseCSV(lines[i]);
...
console.log('Parsing Personal Savings & Investments (line 8485)...');

// Set default date to last day of month for savings
let savingsDate = '2023-04-30';

for (let i = 8484; i < 8486; i++) {
  const row = parseCSV(lines[i]);
...
// Write output file
const outputPath = path.join(__dirname, 'april-2023-PARSED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file with exchange rate
const metadataPath = path.join(__dirname, 'april-2023-METADATA.json');
const metadata = {
  month: 'April 2023',
