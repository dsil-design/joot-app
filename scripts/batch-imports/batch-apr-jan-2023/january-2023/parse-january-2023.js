 ...
 * Batch: Apr-Mar-Feb-Jan 2023 (Month 4 of 4)
 *
...
console.log('Batch: Apr-Mar-Feb-Jan 2023 (Month 4 of 4)\n');
...
console.log('Parsing Expense Tracker (lines 9049-9261)...');
let currentDate = null;

for (let i = 9048; i < 9262; i++) {
...
console.log('Parsing Gross Income Tracker (lines 9276-9280)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2023-01-30';

for (let i = 9275; i < 9281; i++) {
  const row = parseCSV(lines[i]);
...
console.log('Parsing Personal Savings & Investments (line 9289)...');

// Set default date to last day of month for savings
let savingsDate = '2023-01-30';

for (let i = 9288; i < 9290; i++) {
  const row = parseCSV(lines[i]);
...
// Write output file
const outputPath = path.join(__dirname, 'january-2023-PARSED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file with exchange rate
const metadataPath = path.join(__dirname, 'january-2023-METADATA.json');
const metadata = {
  month: 'January 2023',
