...
/**
 * JUNE 2021 PARSING SCRIPT
 * Enhanced with 24+ months of historical learnings
 *
 * Protocol: MASTER-IMPORT-PROTOCOL v4.0 + Protocol v2.0
 * Batch: Aug-May 2021 (Month 3 of 4)
 *
 * Line Ranges from CSV:
 * - Expense Tracker: 13925-14119 (~207 lines)
 * - Gross Income Tracker: 14121-14128 (~9 lines)
 * - Personal Savings & Investments: 14130-14133 (~4 lines)
 */

console.log('========================================');
console.log('JUNE 2021 PARSING SCRIPT');
console.log('========================================\n');
console.log('Protocol: MASTER-IMPORT-PROTOCOL v4.0');
console.log('Enhanced with 24+ months of historical learnings');
console.log('Batch: Aug-May 2021 (Month 3 of 4)\n');
...
function parseFullDate(dateStr) {
  ...
  if (match) {
    ...
    if (monthName === 'August' && year !== '2021') {
      console.log(`  ⚠️  DATE TYPO DETECTED: ${dateStr} - Expected June 2021`);
      parsedYear = '2021';
...
// Section 1: Expense Tracker (lines 13925-14119)
console.log('Parsing Expense Tracker (lines 13925-14119)...');
let currentDate = null;

for (let i = 13924; i < 14120; i++) {
  ...
  const transaction = {
    transaction_date: currentDate || '2021-06-30',
    description,
    merchant,
    amount,
    currency,
...
// Section 2: Gross Income Tracker (lines 14121-14128)
console.log('Parsing Gross Income Tracker (lines 14121-14128)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2021-06-30';

for (let i = 14120; i < 14129; i++) {
  const row = parseCSV(lines[i]);
...
// Section 3: Personal Savings & Investments (lines 14130-14133)
console.log('Parsing Personal Savings & Investments (lines 14130-14133)...');

// Set default date to last day of month for savings
let savingsDate = '2021-06-30';

for (let i = 14129; i < 14134; i++) {
  const row = parseCSV(lines[i]);
...
// Write output file
const outputPath = path.join(__dirname, 'june-2021-CORRECTED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file
const metadataPath = path.join(__dirname, 'june-2021-METADATA.json');
const metadata = {
  month: 'June 2021',
...
console.log('1. Review june-2021-CORRECTED.json');
