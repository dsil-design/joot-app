...
/**
 * APRIL 2022 PARSING SCRIPT
 * Enhanced with 25+ months of historical learnings
 *
 * Protocol: MASTER-IMPORT-PROTOCOL v3.0 + Protocol v2.0
 * Batch: Aug-Jul-Jun-April 2022 (Month 4 of 4)
 *
 * Line Ranges from CSV:
 * - Expense Tracker: 11423-11698 (~175 lines)
 * - Gross Income Tracker: 11700-11712 (~11 lines)
 * - Personal Savings & Investments: 11714-11729 (~5 lines)
 */

console.log('========================================');
console.log('APRIL 2022 PARSING SCRIPT');
console.log('========================================\n');
console.log('Protocol: MASTER-IMPORT-PROTOCOL v3.0');
console.log('Enhanced with 25+ months of historical learnings');
console.log('Batch: Aug-Jul-Jun-April 2022 (Month 4 of 4)\n');
...
function parseFullDate(dateStr) {
  ...
  if (match) {
    ...
    if (monthName === 'April' && year !== '2022') {
      console.log(`  ⚠️  DATE TYPO DETECTED: ${dateStr} - Expected April 2022`);
      parsedYear = '2022';
...
// Section 1: Expense Tracker (lines 11423-11698)
console.log('Parsing Expense Tracker (lines 11423-11698)...');
let currentDate = null;

for (let i = 11422; i < 11699; i++) {
  ...
  const transaction = {
    transaction_date: currentDate || '2022-04-30',
    description,
    merchant,
    amount,
    currency,
...
// Section 2: Gross Income Tracker (lines 11700-11712)
console.log('Parsing Gross Income Tracker (lines 11700-11712)...');

// Set default date to last day of month for income without specific dates
let incomeDate = '2022-04-30';

for (let i = 11699; i < 11713; i++) {
  const row = parseCSV(lines[i]);
...
// Section 3: Personal Savings & Investments (lines 11714-11729)
console.log('Parsing Personal Savings & Investments (lines 11714-11729)...');

// Set default date to last day of month for savings
let savingsDate = '2022-04-30';

for (let i = 11713; i < 11730; i++) {
  const row = parseCSV(lines[i]);
...
const expectedCount = 222; // Estimated from line ranges (120-130 target)
...
// Write output file
const outputPath = path.join(__dirname, 'april-2022-CORRECTED.json');
fs.writeFileSync(outputPath, JSON.stringify(transactions, null, 2));

// Write metadata file
const metadataPath = path.join(__dirname, 'april-2022-METADATA.json');
const metadata = {
  month: 'April 2022',
...
console.log('1. Review april-2022-CORRECTED.json');
