...
-- Email sub-orders table - per-shipment breakdown for vendors (e.g. Amazon) that
-- send one order email summarizing multiple sub-orders that each post to the
-- credit card as a separate charge. The parent email_transactions row keeps
-- the grand total; sub-orders carry per-charge amount + match linkage.
CREATE TABLE public.email_sub_orders (
  ...
  -- Per-sub-order match linkage (mirrors email_transactions columns)
  matched_transaction_id UUID REFERENCES public.transactions(id) ON DELETE SET NULL,
  match_confidence INTEGER CHECK (match_confidence IS NULL OR (match_confidence >= 0 AND match_confidence <= 100)),
  match_method TEXT CHECK (match_method IS NULL OR match_method IN ('auto', 'manual', 'cross_source')),
  matched_at TIMESTAMPTZ,
...
CREATE INDEX idx_email_sub_orders_parent ON public.email_sub_orders(email_transaction_id);
CREATE INDEX idx_email_sub_orders_user_unmatched ON public.email_sub_orders(user_id)
  WHERE matched_transaction_id IS NULL;
CREATE INDEX idx_email_sub_orders_matched_tx ON public.email_sub_orders(matched_transaction_id)
  WHERE matched_transaction_id IS NOT NULL;

CREATE TRIGGER update_email_sub_orders_updated_at BEFORE UPDATE ON public.email_sub_orders
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

ALTER TABLE public.email_sub_orders ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own email sub orders" ON public.email_sub_orders
  FOR SELECT USING ((select auth.uid()) = user_id);
CREATE POLICY "Users can insert own email sub orders" ON public.email_sub_orders
  FOR INSERT WITH CHECK ((select auth.uid()) = user_id);
CREATE POLICY "Users can update own email sub orders" ON public.email_sub_orders
  FOR UPDATE USING ((select auth.uid()) = user_id);
CREATE POLICY "Users can delete own email sub orders" ON public.email_sub_orders
  FOR DELETE USING ((select auth.uid()) = user_id);

-- Keep parent email_transactions.status in sync with sub-order match state.
-- Parent is `matched` only when every sub-order is matched; otherwise it
-- reverts to `waiting_for_statement`. Never overrides terminal states
-- (skipped/imported) or pending_review.
