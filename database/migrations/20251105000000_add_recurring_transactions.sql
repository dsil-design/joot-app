...
CREATE INDEX idx_templates_user_id ON public.transaction_templates(user_id);
CREATE INDEX idx_templates_active ON public.transaction_templates(is_active) WHERE is_active = true;
CREATE INDEX idx_templates_frequency ON public.transaction_templates(frequency);
CREATE INDEX idx_templates_dates ON public.transaction_templates(start_date, end_date);

-- RLS Policies for transaction_templates
ALTER TABLE public.transaction_templates ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own templates" ON public.transaction_templates
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own templates" ON public.transaction_templates
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own templates" ON public.transaction_templates
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own templates" ON public.transaction_templates
...
CREATE INDEX idx_template_tags_template_id ON public.template_tags(template_id);
...
CREATE POLICY "Users can view template tags" ON public.template_tags
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Users can insert template tags" ON public.template_tags
...
CREATE POLICY "Users can delete template tags" ON public.template_tags
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM public.transaction_templates
      WHERE transaction_templates.id = template_tags.template_id
...
CREATE INDEX idx_month_plans_user_id ON public.month_plans(user_id);
CREATE INDEX idx_month_plans_month_year ON public.month_plans(month_year DESC);
CREATE INDEX idx_month_plans_status ON public.month_plans(status);
CREATE INDEX idx_month_plans_user_month ON public.month_plans(user_id, month_year DESC);

ALTER TABLE public.month_plans ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own month plans" ON public.month_plans
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own month plans" ON public.month_plans
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own month plans" ON public.month_plans
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own month plans" ON public.month_plans
  FOR DELETE USING (auth.uid() = user_id);
...
CREATE INDEX idx_expected_transactions_user_id ON public.expected_transactions(user_id);
CREATE INDEX idx_expected_transactions_month_plan ON public.expected_transactions(month_plan_id);
CREATE INDEX idx_expected_transactions_template ON public.expected_transactions(template_id);
CREATE INDEX idx_expected_transactions_status ON public.expected_transactions(status);
CREATE INDEX idx_expected_transactions_date ON public.expected_transactions(expected_date);
CREATE INDEX idx_expected_transactions_matched ON public.expected_transactions(matched_transaction_id) WHERE matched_transaction_id IS NOT NULL;
CREATE INDEX idx_expected_transactions_user_month ON public.expected_transactions(user_id, expected_date);

ALTER TABLE public.expected_transactions ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own expected transactions" ON public.expected_transactions
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own expected transactions" ON public.expected_transactions
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own expected transactions" ON public.expected_transactions
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own expected transactions" ON public.expected_transactions
  FOR DELETE USING (auth.uid() = user_id);
...
CREATE TABLE public.expected_transaction_tags (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
...
CREATE INDEX idx_expected_transaction_tags_expected_id ON public.expected_transaction_tags(expected_transaction_id);
CREATE INDEX idx_expected_transaction_tags_tag_id ON public.expected_transaction_tags(tag_id);

ALTER TABLE public.expected_transaction_tags ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view expected transaction tags" ON public.expected_transaction_tags
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Users can insert expected transaction tags" ON public.expected_transaction_tags
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.expected_transactions
      WHERE expected_transactions.id = expected_transaction_tags.expected_transaction_id
...
CREATE POLICY "Users can delete expected transaction tags" ON public.expected_transaction_tags
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM public.expected_transactions
      WHERE expected_transactions.id = expected_transaction_tags.expected_transaction_id
...
CREATE INDEX IF NOT EXISTS idx_transactions_expected_id ON public.transactions(expected_transaction_id)
